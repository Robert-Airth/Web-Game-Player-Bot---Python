...
This was the first time I learned about implementing multiple threads, so it took me a while to research and develop my code to work properly. It now runs one thread listening for the ESC and when triggered, executes os._exit() to kill the main thread (bot) from a post-fork thread (key-listener) (I think that my understanding of how this works is correct since os.exit() didn't work properly). This took several more hours to figure out. I may want to add some other key-listening functionality like being able to pause/resume the bot's actions.
